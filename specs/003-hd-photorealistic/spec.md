...
- **Select balance tuning**: While the core gameplay design is unchanged, select values (baseHp, turret HP, blaster range, wallDamage) were tuned alongside the HD upgrade to maintain intended difficulty at the new resolution. Wave compositions and economy values remain the same.
